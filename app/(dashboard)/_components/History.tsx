...
function History({ userSettings }: { userSettings: UserSettings }) {
    ...
    return (
        <div className="px-4">
            ...
            <Card className="col-span-12 mt-12 w-full min-h-100 mb-12">
                        ...
                        )}
                        {
                            !dataAvailable && (
                                <Card className='flex h-[300px] flex-col items-center justify-center bg-background'>
                                    Sem movimentações para o perído escolhido
                                    <p className='text-sm text-muted-foreground'>
                                        Selecione um período diferente ou crie uma movimentação
                                    </p>
                                </Card>
                            )
                        }
...
function CustomTooltip({ active, payload, formatter, timeframe }: any) {
    if (!active || !payload || payload.length === 0) return null

    const data = payload[0].payload
    const { expense, income, monthBalance } = data;
    ...
    return (<div className='min-w-[300px] rounded border bg-background p-4'>
        ...
        <TooltipRow
            formatter={formatter}
            label="Saldo"
            value={timeframe === 'month' ? (monthBalance) : (income - expense)}
            bgColor="bg-gray-100"
